...
function seedTestData() {
    ...
    testDb.prepare("INSERT INTO users (keycloak_id, email, name, role) VALUES ('test-keycloak-id', 'test@stu.kau.edu.sa', 'Test User', 'student')").run();
    // Create an admin user
    testDb.prepare("INSERT INTO users (keycloak_id, email, name, role) VALUES ('admin-keycloak-id', 'admin@kau.edu.sa', 'Admin User', 'admin')").run();
    // Create a club
    testDb.prepare("INSERT INTO clubs (name, name_ar, description, description_ar) VALUES ('Tech Club', 'نادي التقنية', 'A tech club', 'نادي تقني')").run();
    // Create a published event
    testDb.prepare(`INSERT INTO events (club_id, title, title_ar, description, description_ar, location, starts_at, ends_at, capacity, status, created_by)
     VALUES (1, 'Tech Talk', 'محادثة تقنية', 'A talk about tech', 'حديث عن التقنية', 'Room 101', '2099-01-01T10:00:00', '2099-01-01T12:00:00', 50, 'published', 2)`).run();
    // Create a draft event
    testDb.prepare(`INSERT INTO events (club_id, title, title_ar, description, description_ar, location, starts_at, ends_at, capacity, status, created_by)
     VALUES (1, 'Draft Event', 'حدث مسودة', 'A draft', 'مسودة', 'Room 102', '2099-02-01T10:00:00', '2099-02-01T12:00:00', 10, 'draft', 2)`).run();
    // Create a full-capacity event (capacity=1)
    testDb.prepare(`INSERT INTO events (club_id, title, title_ar, description, description_ar, location, starts_at, ends_at, capacity, status, created_by)
     VALUES (1, 'Full Event', 'حدث ممتلئ', 'A full event', 'حدث ممتلئ', 'Room 103', '2099-03-01T10:00:00', '2099-03-01T12:00:00', 1, 'published', 2)`).run();
}
beforeEach(() => {
...
describe('Event registration flow', () => {
    ...
    describe('failure cases', () => {
        ...
        it('rejects registration when event is at full capacity', async () => {
            ...
            const adminToken = (0, setup_1.generateAdminToken)();
            await (0, supertest_1.default)(app)
                .post('/api/events/3/register')
                .set('Authorization', `Bearer ${adminToken}`);
            // Now try with student
            const studentToken = (0, setup_1.generateTestToken)();
            const res = await (0, supertest_1.default)(app)
                .post('/api/events/3/register')
...
//# sourceMappingURL=event-registration.test.js.map