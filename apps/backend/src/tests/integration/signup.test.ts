import { createTestDb, TEST_JWT_SECRET } from '../setup';
...
jest.mock('../../services/keycloakAdmin.service', () => ({
  createKeycloakUser: (...args: any[]) => mockCreateKeycloakUser(...args),
}));
...
describe('POST /api/auth/signup', () => {
  ...
  describe('audit logging', () => {
    ...
    it('logs duplicate email conflict in audit_logs', async () => {
      const err = new Error('Duplicate') as Error & { status: number };
      err.status = 409;
      mockCreateKeycloakUser.mockRejectedValueOnce(err);
